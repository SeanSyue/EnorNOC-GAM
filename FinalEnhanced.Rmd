...
```{r}
gam_new_te <- gam(Load ~ te(Daily, Weekly,
                        k = c(period, 7),
                        bs = c("cr", "ps")),
              data = matrix_new,
              family = gaussian)
...
```{r, fig.width=12}
datas <- rbindlist(list(data_new[, .(value, date_time)],
                        data.table(value = gam_new_te$fitted.values,
                                   data_time = data_new[, date_time])))
datas[, type := c(rep("Real", nrow(data_new)), rep("Fitted", nrow(data_new)))]
...
## 第一個GAM模型`gam_new_t2`
```{r}
gam_new_t2 <- gam(Load ~ t2(Daily, Weekly,
                        k = c(period, 7),
                        bs = c("cr", "ps"),
                  full = TRUE),
              data = matrix_new,
...
## `gam_new_t2`的summary
```{r}
summary(gam_new_t2)$r.sq
summary(gam_new_t2)$sp.criterion
```

## `gam_new_t2`的擬合狀況圖像
```{r, fig.width=12}
datas <- rbindlist(list(data_new[, .(value, date_time)],
                        data.table(value = gam_new_t2$fitted.values,
                                   data_time = data_new[, date_time])))
datas[, type := c(rep("Real", nrow(data_new)), rep("Fitted", nrow(data_new)))]
